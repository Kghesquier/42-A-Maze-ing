...
from output import save_maze


def render_maze(stdscr: curses.window,
                maze: MazeGenerator, output_name: str) -> None:
    """Render the maze with a lava color theme.

    Red walls, yellow solution path, white '42' logo.

    Args:
        stdscr:      Curses window provided by curses.wrapper.
        maze:        MazeGenerator instance used to generate and solve.
        output_name: Path of the output file to write.

    Keys:
        1: Regenerate a new maze.
        2: Toggle the solution path on/off.
        3: Swap wall and 42-pattern colors.
    ...
    try:
        grid = maze.generate()
        path = maze.solve()
        save_maze(output_name, maze, path)
    ...
    while True:
        try:
            ...
            if total_width > term_width or total_height > term_height:
                stdscr.clear()
                stdscr.addstr(0, 0, "Terminal is too small... Please resize "
                              "or press 'q' to quit.")
                stdscr.refresh()
            ...
            stdscr.addstr(total_height + 2, 0, "1. Generate a new maze")
            stdscr.addstr(total_height + 3, 0, "2. Show/Hide "
                          "the path from entry to exit")
            stdscr.addstr(total_height + 4, 0, "3. Swap the maze colors "
                          "(walls & logo)")
